...
#Genderidentity did not have to change to gender b/c it is already called gender. 
...
#str function displays what is a charcater versus a number
str(tbl_clean)
#Once we know that id, gender and state are character types, we can change these to factors
...
# sort the output and then use indexing to print the 3 most common response (not including missing values)
...
tbl_clean$gender4 <- recode(tbl_clean$gender, "[1]"="A", "[2]"="B", "[5]"="C", .default="other")
...
#line 35- edu : edu_14 (education: 1 "elementary" 2 "junior high" 3 "some high school" 4 "HS grad" 5 "some college" 6 "associate's" 7 "bachelor's" 8 "some grad" 9 "MA" 10 "JD" 11 "MD" 12 "PHD" 13 "other advanced" 14 "MBA")

tbl_clean$edu7 <- recode(tbl_clean$edu, "1"="no highschool", "2"="some highschool", "3"="some highschool", "4"="highschool graduate", "5"="some college", "6"="postsecondary degree", "7"="postsecondary degree", "8"="masters (MA & MBA)", "9"="masters(MA & MBA)", "14"="masters (MA & MBA)", "10"="advanced degree", "11"="advanced degree", "12"="advanced degree","13"="advanced degree") 


# mutating variables ---------------------------------------------  
# rewrite the above recoding steps so that they both occur within a single call of the mutate function

tbl_clean <- mutate(tbl_clean, gender4 = recode(gender, "[2]" = "B", "[1]" = "A", "[5]" = "C", .default = "Other"), edu7 = recode(as.factor(edu), '1'='no highschool','2' = 'some highschool', '3' = 'some highschool', '4'='highschool graduate', '5' = 'some college', '6' = 'postsecondary degree', '7' = 'postsecondary degree', '8' = 'masters (MA & MBA)', '9'= 'masters (MA&MBA)' , '14' = 'masters (MA & MBA)', '10' = 'advanced degree', '11'= 'advanced degree', '12' = 'advanced degree', '13' = 'advanced degree'))
...
WhiteMen <- filter(tbl_clean, race==6, gender=="[1]")
MeanWhiteMen <-mean(WhiteMen$bias)
print(MeanWhiteMen)
...
WhiteWomen <- filter(tbl_clean, race==6, gender=="[2]")
MeanWhiteWomen <-mean(WhiteWomen$bias)
print(MeanWhiteWomen)

#advanced degree holders who are men
#advanced degree
#men:1

AdvanceddegreeMen <- filter(tbl_clean, edu7=="advanced degree", gender=="[1]")
MeanAdvanceddegreeMen <- mean(AdvanceddegreeMen$bias)
print(MeanAdvanceddegreeMen)

#high school graduates who are men
#high school graduates
#men:1

HighschoolgraduateMen <- filter(tbl_clean, edu7=="highschool graduate", gender=="[1]")
MeanHighschoolgraduateMen <- mean(HighschoolgraduateMen$bias)
print(MeanHighschoolgraduateMen)
